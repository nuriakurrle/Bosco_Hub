...
const HOUSES = {
  ...
  // Zeltplatz: 4 Gruppenzelte. `beds` = Personen-Schätzung (~12 je Gruppenzelt)
  // für die Belegungsrechnung; `tents` = die eigentliche Kapazität.
  zeltplatz: { beds: 48, tents: 4, seminarRooms: 0, referentSlots: 1 },
};
...
function houseKey(houseName = "") {
  ...
  if (h.includes("bildungszentrum") || h.includes("jugendherberge")) return "jugendherberge";
  if (h.includes("gästehaus") || h.includes("gaestehaus") || h.includes("gästezentrum") || h.includes("gaestezentrum") || h.includes("aktionszentrum")) return "aktionszentrum";
  if (h.includes("zeltplatz") || h.includes("zelt")) return "zeltplatz";
  return null;
}
...
// Programm → Skill-Schlüssel (für qualifizierte Referent:innen).
const SKILL_KEYS = ["orientierung", "schullandheim", "sommerfreizeit", "besinnung", "umwelt", "seminar", "gruppenleiter"];
function skillFor(programType = "") {
  const p = programType.toLowerCase();
  return SKILL_KEYS.find((k) => p.includes(k)) || null;
}

export function assessInquiry(item, occupancy = [], staff = []) {
  ...
  // Qualifizierte Referent:innen für dieses Format (Skills).
  const skill = skillFor(programType);
  const qualified = skill ? staff.filter((s) => (s.skills || "").toLowerCase().includes(skill)).length : null;
  const safety = {
    gender,
    resource: {
      referentsNeeded,
