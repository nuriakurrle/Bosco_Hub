...
import { getBookings, getAllTasksDone, getHouses } from "@/lib/bookings";
...
export default async function BuchungenPage() {
  ...
  let error = null;
  try {
    [bookings, staff, tasksDone, houses] = await Promise.all([getBookings(), getStaff(), getAllTasksDone(), getHouses()]);
  ...
  return (
    <Shell staff={staff} me={me} active="buchungen">
      {error ? (
        ...
        <BookingsView bookings={bookings} tasksDone={tasksDone} me={me} houses={houses} />
      )}
    </Shell>
  );
}
